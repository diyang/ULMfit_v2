...
class RNN_bidirect_build_graph:
    ...
    def build_graph(self):
        ...
        with tf.variable_scope("rnn_layer"):
            ...
            for i in range(self.sequence_length):
                ...
                W_omega = tf.get_variable('W_omega_'+str(i), [self.state_size*2, self.state_size])
                b_omega = tf.get_variable('b_omega_'+str(i), [self.state_size])
                u_omega = tf.get_variable('u_omega_'+str(i), [self.state_size])
                v = tf.tanh(tf.tensordot(rnn_outputs, W_omega, axes=1)+b_omega)
                vu = tf.tensordot(v, u_omega, axes=1)
                alphas = tf.nn.softmax(vu)
                rnn_outputs_attn = tf.reshape(tf.reduce_sum(rnn_outputs * tf.expand_dims(alphas, -1), 1), [self.batch_size, 1, self.state_size*2])
                attn_layers.append(rnn_outputs_attn)
            rnn_outputs_attn = tf.concat(attn_layers, 1)
        ...
        with tf.variable_scope('word_prediction'):
            ...
            W2 = tf.get_variable('W2', [self.state_size, self.num_words])
            b2 = tf.get_variable('b2', [self.num_words], initializer=tf.constant_initializer(0.0))
            rnn_outputs_attn_reshape = tf.reshape(rnn_outputs_attn, [-1, self.state_size*2])
        ...
        with tf.variable_scope('topic_sigmoid'):
            # Hierarchical Attention Layer
            W_omega_topic = tf.get_variable('W_omega_topic', [self.state_size*2, self.state_size])
            b_omega_topic = tf.get_variable('b_omega_topic', [self.state_size])
            u_omega_topic = tf.get_variable('u_omega_topic', [self.state_size])
            v_topic = tf.tanh(tf.tensordot(rnn_outputs_attn, W_omega_topic, axes=1)+b_omega_topic)
            vu_topic = tf.tensordot(v_topic, u_omega_topic, axes=1)
            alphas_topic = tf.nn.softmax(vu_topic)
            rnn_outputs_attn_topic = tf.reduce_sum(rnn_outputs_attn * tf.expand_dims(alphas_topic, -1), 1)

            # Dense Layers
            rnn_outputs_lst = tf.squeeze(tf.slice(rnn_outputs, [0, (self.sequence_length-1), 0], [self.batch_size, 1, self.state_size*2]))
            rnn_outputs_concat = tf.concat([rnn_outputs_lst, rnn_outputs_attn_topic], 1)
            ...
            topic_logits = tf.matmul(fc2, W_s3) + b_s3
            topic_predictions = tf.nn.sigmoid(topic_logits, name='predictions_topic')
            topic_cross_entropy = tf.nn.weighted_cross_entropy_with_logits(labels=y_topic, logits=topic_predictions, pos_weight = tf_pos_weight, name="cross_entropy_topic")
            #topic_cross_entropy = tf.nn.sigmoid_cross_entropy_with_logits(labels=y_topic, logits=topic_predictions, name="cross_entropy_topic")
            topic_cross_entropy_mean = tf.reduce_mean(topic_cross_entropy, name="cross_entropy_topic_mean")

        freeze_train_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="topic_sigmoid")
        topic_train_step_freeze = tf.train.AdamOptimizer(tf_learning_rate).minimize(topic_cross_entropy_mean, var_list=freeze_train_vars)
        topic_train_step_unfreeze = tf.train.AdamOptimizer(tf_learning_rate).minimize(topic_cross_entropy_mean)
        ...
        return dict(
            ...
            total_loss_topic = topic_cross_entropy_mean,
            train_step_topic_freeze = topic_train_step_freeze,
            train_step_topic_unfreeze = topic_train_step_unfreeze,
            preds_topic = topic_predictions,

            freeze_check = rnn_outputs_attn_reshape,
    ...
    def train_finetune_model(self, data, num_epochs=0, learning_rate=1e-4, num_epochs2=None, learning_rate2=1e-4,
                             ...
                             save=False, pretrain_model=None, training_resume=False, debug=True, pos_weight=None):
        tf.set_random_seed(2345)
        ...
        with tf.Session() as sess:
            ...
            total_loss_op = self.graph['total_loss_topic']
            pred_op = self.graph['preds_topic']
            ...
            for epoch in range(num_epochs_combine):
                ...
                if epoch < num_epochs:
                    train_op = self.graph['train_step_topic_freeze']
                    learning_rate_feed = learning_rate
                    dropout_feed = dropout
                else:
                    train_op = self.graph['train_step_topic_unfreeze']
                ...
                for step in range(steps_per_train_epoch):
                    ...
                    feed_dict = {self.graph['x']: batch_data, self.graph['y_topic']: batch_labels,
                                 self.graph['embeddings']: data['embedding'],
                                 self.graph['pos_weight']: pos_weight,
    ...
    def feedforward(self, data, model_path=None):
        ...
        for step in range(batch_steps):
            ...
            pred = self.sess.run(self.graph['preds_topic'], feed_dict)
            preds.append(pred)
